...
const SiderBar = ({ onNavigate = () => {} }) => {
  ...
  const sidebarGuides = useMemo(() => {
    const guides = [];
    const tokenVisible = workspaceItems.some((item) => item.itemKey === 'token');
    ...
    if (topupVisible && (!logVisible || logDone) && (!tokenVisible || tokenDone)) {
      guides.push({
      ...
      if (topupVisible && (!logVisible || logDone) && (!tokenVisible || tokenDone) && topupDone) {
        guides.push({
